...
export class PlantComponent {
  ...
  getDefaultSet(limit,offset){
    this.subscriber = this.config.getPlants(limit,offset).subscribe(
      data => {
        if (data == null) {
  ...
  addPlant(mode = 'add') {
    this._dialog(ADD_UPDATE_DIALOG_OPTIONS(230,400), mode, {});
  }
  ...
  update(row, mode) {
    this._dialog(
      mode === MODE.UPDATE ? ADD_UPDATE_DIALOG_OPTIONS(230,400) : DELETE_DIALOG_OPTIONS,
      mode,
      Object.assign({}, row)
    );
  }
  ...
  private _dialog(options: DIALOG_OPTIONS, mode: string, data) {
    ...
    this.dialogRef.afterClosed().subscribe((data: any) => {
      if (typeof data != 'string') {
        this.subscriber = this.config.getPlants(0,0).subscribe(
          data => {
            if (data == null) {
